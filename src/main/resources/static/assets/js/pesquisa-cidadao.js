						...
						if (cns == "") {
							...
							if (validaCns.test(cns)) {
								...
								$.getJSON(rootUrl + "/localCns?cns=" + cns, function(data){
									...
									$("#resultado-pesquisa").addClass("text-success").text("Cidadão encontrado na <strong>Base de Dados Local</strong>. Confira os dados no formulário abaixo.");
									...
									$.getJSON(rootUrl + "/cadsusCns?cns=" + cns, function(data){
										...
										$("#codigonacionalidade").val(data["soap:Envelope"]["S:Body"].PRPA_IN201306UV02.controlActProcess.subject.registrationEvent.subject1.patient.patientPerson.birthPlace.addr.country);
										...
										for(var i = 0; i < outrasIds; i++){
											...
											if(data["soap:Envelope"]["S:Body"].PRPA_IN201306UV02.controlActProcess.subject.registrationEvent.subject1.patient.patientPerson.asOtherIDs[i].id[0].root == "2.16.840.1.113883.13.237"){
												$("#cpf-form").val(data["soap:Envelope"]["S:Body"].PRPA_IN201306UV02.controlActProcess.subject.registrationEvent.subject1.patient.patientPerson.asOtherIDs[i].id[0].extension);
											}
										};
